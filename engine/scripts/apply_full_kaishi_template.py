...
export const EtymologyTemplate: React.FC<EtymologyConfig> = ({
  ...
  screenTimestamps = {
    screen1EndFrame: 225,
    screen2EndFrame: 799,
    screen3EndFrame: 1553,
    lessonTotalFrames: 2031,
  },
  animationTimestamps,
  lessonDurationInFrames = 2031,
  ...
  const char1 = character.charAt(0) || '开';
  const char2 = character.charAt(1) || '始';
  ...
  const charSpacing = isKaishi ? 120 : 150;
  const bangX =
    interpolate(morph1To2, [0, 1], [-charSpacing, 0]) +
    interpolate(morph2To3, [0, 1], [0, -800]) +
    interpolate(morph3To4, [0, 1], [0, 800 - charSpacing]);

  const zhuX =
    interpolate(morph1To2, [0, 1], [charSpacing, 800]) +
    interpolate(morph2To3, [0, 1], [0, -800]) +
  ...
  const anim = animationTimestamps || {
    screen1: {
      ...
      clothMention: { startFrame: 134, endFrame: 144 },
      wallMention: { startFrame: 163, endFrame: 176 },
      altarMention: { startFrame: 195, endFrame: 216 },
      muscleMention: { startFrame: 195, endFrame: 216 },
    },
    screen2: {
      startFrame: screen1EndFrame,
      endFrame: screen2EndFrame,
      topBang: { startFrame: screen1EndFrame, endFrame: 596 },
      bottomJin: { startFrame: 596, endFrame: screen2EndFrame },
      wholeBang: { startFrame: 596, endFrame: screen2EndFrame },
    },
    screen3: {
      startFrame: screen2EndFrame,
      endFrame: screen3EndFrame,
      wholeZhuIntro: { startFrame: screen2EndFrame, endFrame: 1008 },
      leftQie: { startFrame: 1008, endFrame: 1112 },
      rightLi: { startFrame: 1194, endFrame: 1272 },
      wholeZhuOutro: { startFrame: 1272, endFrame: screen3EndFrame },
    },
    screen4: {
      startFrame: screen3EndFrame,
      endFrame: lessonDurationInFrames,
      bangHighlightEndFrame: 1749,
    },
  };
  ...
  let spot2X = 540;
  let spot2Y = isKaishi ? 240 : isJieshao ? 250 : isWangji ? 210 : isAiqing ? 220 : isPengyou ? 300 : 210;
  let spot2R = isKaishi ? 180 : 120;
  ...
  let spot3X = 540;
  let spot3Y = isKaishi ? 240 : 300;
  let spot3R = isKaishi ? 180 : 230;
    ...
    <AbsoluteFill style={{ backgroundColor: COLORS.bg, fontFamily: 'Roboto, sans-serif', overflow: 'hidden' }}>
      <Sequence from={0} durationInFrames={lessonDurationInFrames}>
        ...
        <AbsoluteFill style={{ padding: '30px 40px', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 10 }}>
          ...
          <div style={{ marginTop: 20, height: 170, textAlign: 'center', width: '100%', display: 'flex', justifyContent: 'center' }}>
            {isScreen1 && (
              <div style={{
                backgroundColor: '#0F172A',
                padding: '24px 50px',
                ...
                <h2 style={{ fontFamily: FONTS.display, fontSize: 56, color: '#FFFFFF', margin: 0, lineHeight: 1.2 }}>
                  ...
                  {isKaishi ? 'Contain a Gate & New Life?' : isJieshao ? 'Contain a Person & Silk?' : isWangji ? 'Contain Disappearing & Words?' : isAiqing ? 'Contain a Friend & Heart?' : isPengyou ? 'Feature 2 Moons & 2 Hands?' : 'Contain Cloth & Muscle?'}
                </h2>
              </div>
            )}
            {isScreen2 && (
              <h2 style={{ fontFamily: FONTS.display, fontSize: 50, color: '#0F172A', margin: 0, lineHeight: 1.2 }}>
                Character 1: <span style={{ color: '#FF6F59' }}>{char1} ({isKaishi ? 'kāi' : isJieshao ? 'jiè' : isWangji ? 'wàng' : isAiqing ? 'ài' : isPengyou ? 'péng' : 'bāng'})</span> — {isKaishi ? 'Opening the Gate' : isJieshao ? 'Go-Between' : isWangji ? 'Disappearing Heart' : isAiqing ? 'Hand Embracing Friend' : isPengyou ? 'Twin Companions' : 'Protective Backing'}
              </h2>
            )}
            {isScreen3 && (
              <h2 style={{ fontFamily: FONTS.display, fontSize: 50, color: '#0F172A', margin: 0, lineHeight: 1.2 }}>
                Character 2: <span style={{ color: '#FF6F59' }}>{char2} ({isKaishi ? 'shǐ' : isJieshao ? 'shào' : isWangji ? 'jì' : isAiqing ? 'qíng' : isPengyou ? 'yǒu' : 'zhù'})</span> — {isKaishi ? 'New Life & Origin' : isJieshao ? 'Linking Thread' : isWangji ? 'Recording Words' : isAiqing ? 'Youthful Heart' : isPengyou ? 'Helping Hands' : 'Muscle Power'}
              </h2>
            )}
            {isScreen4 && (
              <h2 style={{ fontFamily: FONTS.display, fontSize: 52, color: '#0F172A', margin: 0, lineHeight: 1.2 }}>
                Synthesis: <span style={{ color: '#FF6F59' }}>{character}</span> = {isKaishi ? 'Opening Gates + Giving Birth!' : isJieshao ? 'Connecting Two Parties!' : isWangji ? 'Disappearing from Memory!' : isAiqing ? 'Blossoming Affection!' : isPengyou ? 'Companions + Helping Hands!' : 'Protection + Muscle!'}
              </h2>
            )}
          </div>
...
print("Applied pixel-perfect layout adjustments!")
