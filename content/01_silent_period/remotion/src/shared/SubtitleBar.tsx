...
import { COLORS, FONTS, FPS, SPRING_GENTLE, getCurrentWord } from './constants';

// ────────────────────────────────────────────────────────────
// SUBTITLE BAR — highlights current word using word-level timing
// Fixed: uses LOCAL frame for spring entrance, not absolute video time
// ────────────────────────────────────────────────────────────
...
export const SubtitleBar: React.FC<SubtitleBarProps> = ({ text, time, startTime, endTime }) => {
  // Local frame within this subtitle's lifetime
  const localFrame = Math.max(0, Math.round((time - startTime) * FPS));
  const localEndFrame = Math.max(0, Math.round((endTime - time) * FPS));

  // Entrance spring (only first ~15 frames)
